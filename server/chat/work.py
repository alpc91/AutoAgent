...
class Work:
    def __init__(self, context_variables: dict):
        ...
        self.base_info_messages = self.rag.query(self.sys_messages[self.stage % 4], "base_info", top_k=2)['documents']
        self.base_info_messages = "\n".join(self.base_info_messages)
        self.history_messages = self.rag.query(self.sys_messages[self.stage % 4], "history", top_k=2)['documents']
        self.history_messages = "\n".join(self.history_messages)
        self.trick_messages = self.rag.query(self.sys_messages[self.stage % 4], "trick", top_k=2)['documents']
        self.trick_messages = "\n".join(self.trick_messages)
    ...
    def query(self, message, callback, debug: bool = True):
        ...
        if message=="" and self.workflow_form:
        ...
        match agent_name:
            case "Workflow Generator Agent":
                ...
                if self.workflow_form:
                    ...
                    requirements = self.base_info_messages+'\n'+self.history_messages+'\n'+self.trick_messages+'\n'+self.sys_messages[self.stage % 4]+'\n'+message
                
                # print(f"实例要求:\n {requirements}\n\n")

                reasoning_content, workflow, messages = self.workflow_generating(self.workflow_generator, self.client, self.messages, self.context_variables, requirements, debug)
                ...
                workflow_form, output_json_form, messages = self.workflow_profiling(self.workflow_former, self.client, self.messages, self.context_variables, debug)
                ...
                if workflow_form is None:
                    ...
                    return
                self.agent = self.workflow_generator 
                self.context_variables["workflow_form"] = workflow_form
                ...
                self.last_message = '如果前端展示的工作流符合你的要求，请按"Enter"回车；否则，请提出具体修改建议。'
                self.messages = messages
                callback(f"生成的json为:")
