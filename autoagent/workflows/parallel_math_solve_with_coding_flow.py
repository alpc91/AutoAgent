...
async def on_start(event: EventInput, global_ctx):
    print("start the workflow:" + 'parallel_math_solve_with_coding')
@default_drive.listen_group([on_start])
async def math_solver_event(event: EventInput, global_ctx):
    inputs = [{'key': 'math_problem', 'description': 'The mathematical problem to be solved analytically.'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    
    messages = global_ctx.get('messages', [])
    task = 'Solve the mathematical problem using analytical methods'
    outputs = [{'key': 'solver_solution', 'description': 'Analytical solution generated by the Math Solver Agent', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
...
@default_drive.listen_group([on_start])
async def coding_solver_event(event: EventInput, global_ctx):
    inputs = [{'key': 'math_problem', 'description': 'The mathematical problem to be solved programmatically'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    
    messages = global_ctx.get('messages', [])
    task = 'Create and execute code to solve the mathematical problem'
    outputs = [{'key': 'code_solution', 'description': 'Computational solution generated by the Coding Agent', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
    ...
    for output in outputs:
        ...
        if ans:
            ...
            return result
    raise Exception("No valid answer found")
@default_drive.listen_group([math_solver_event, coding_solver_event])
async def aggregate_event(event: EventInput, global_ctx):
    inputs = [{'key': 'solver_solution', 'description': 'Analytical solution from the Math Solver Agent'}, {'key': 'code_solution', 'description': 'Code-based solution from the Coding Agent'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    ...
    outputs = [{'key': 'final_answer', 'description': 'Final answer determined by majority vote between the two solutions', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
...
@register_workflow(name = 'parallel_math_solve_with_coding')
async def parallel_math_solve_with_coding(system_input: str):
