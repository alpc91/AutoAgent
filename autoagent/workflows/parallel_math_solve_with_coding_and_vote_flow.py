...
async def on_start(event: EventInput, global_ctx):
    print("start the workflow:" + 'parallel_math_solve_with_coding_and_vote')
@default_drive.listen_group([on_start])
async def math_solve(event: EventInput, global_ctx):
    inputs = [{'key': 'math_problem', 'description': 'The mathematical problem to solve.'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    
    messages = global_ctx.get('messages', [])
    task = 'Solve the math problem analytically using mathematical principles.'
    outputs = [{'key': 'solver_result', 'description': 'Analytical solution to the math problem.', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
...
async def coding_solve(event: EventInput, global_ctx):
    ...
    messages = global_ctx.get('messages', [])
    task = 'Solve the math problem by writing and executing code.'
    outputs = [{'key': 'coding_result', 'description': 'Code-based solution to the math problem.', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
    ...
    for output in outputs:
        ...
        if ans:
            ...
            return result
    raise Exception("No valid answer found")
@default_drive.listen_group([math_solve, coding_solve])
async def voting_aggregate(event: EventInput, global_ctx):
    inputs = [{'key': 'solver_result', 'description': 'Analytical solution from Math Solver Agent.'}, {'key': 'coding_result', 'description': 'Code-based solution from Coding Agent.'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    ...
    outputs = [{'key': 'final_answer', 'description': 'Final solution determined by majority vote between analytical and code-based approaches.', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
...
@register_workflow(name = 'parallel_math_solve_with_coding_and_vote')
async def parallel_math_solve_with_coding_and_vote(system_input: str):
