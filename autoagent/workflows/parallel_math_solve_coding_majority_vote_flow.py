...
async def on_start(event: EventInput, global_ctx):
    print("start the workflow:" + 'parallel_math_solve_coding_majority_vote')
@default_drive.listen_group([on_start])
async def solve_analytically(event: EventInput, global_ctx):
    inputs = [{'key': 'math_problem', 'description': 'The math problem to be solved (e.g., equation, word problem, etc.).'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    
    messages = global_ctx.get('messages', [])
    task = 'Analyze and solve the math problem using analytical methods'
...
@default_drive.listen_group([on_start])
async def solve_via_code(event: EventInput, global_ctx):
    inputs = [{'key': 'math_problem', 'description': 'The math problem to be solved (e.g., equation, word problem, etc.).'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    ...
    messages = global_ctx.get('messages', [])
    task = 'Solve the problem by generating and executing code'
    outputs = [{'key': 'solution_code', 'description': 'Code-based solution from Coding Agent', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
    ...
    for output in outputs:
        ...
        if ans:
            ...
            return result
    raise Exception("No valid answer found")
@default_drive.listen_group([solve_analytically, solve_via_code])
async def aggregate_solutions(event: EventInput, global_ctx):
    inputs = [{'key': 'solution_math', 'description': 'Analytical solution generated by Math Solver Agent'}, {'key': 'solution_code', 'description': 'Code-based solution from Coding Agent'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    
    messages = global_ctx.get('messages', [])
    task = 'Aggregate solutions using majority voting algorithm'
    outputs = [{'key': 'final_answer', 'description': 'Final answer selected through majority voting process', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
...
@register_workflow(name = 'parallel_math_solve_coding_majority_vote')
async def parallel_math_solve_coding_majority_vote(system_input: str):
