...
async def on_start(event: EventInput, global_ctx):
    print("start the workflow:" + 'math_parallel_coding_solve_vote')
@default_drive.listen_group([on_start])
async def analytical_solution(event: EventInput, global_ctx):
    inputs = [{'key': 'math_problem', 'description': 'The math problem statement to be solved.'}]
    input_dict = dict()
    for inp in inputs: 
    ...
    outputs = [{'key': 'math_solution', 'description': 'Analytical solution generated by Math Solver Agent', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
...
@default_drive.listen_group([on_start])
async def coding_solution(event: EventInput, global_ctx):
    inputs = [{'key': 'math_problem', 'description': 'The math problem statement to be solved.'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    ...
    outputs = [{'key': 'code_solution', 'description': 'Code-based solution generated by Coding Agent', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
    ...
    for output in outputs:
        ...
        if ans:
            ...
            return result
    raise Exception("No valid answer found")
@default_drive.listen_group([analytical_solution, coding_solution])
async def voting_aggregation(event: EventInput, global_ctx):
    inputs = [{'key': 'math_solution', 'description': 'Analytical solution generated by Math Solver Agent'}, {'key': 'code_solution', 'description': 'Code-based solution generated by Coding Agent'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    
    messages = global_ctx.get('messages', [])
    task = 'Aggregate solutions using majority voting'
    outputs = [{'key': 'final_answer', 'description': 'Final solution determined by majority vote', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
...
@register_workflow(name = 'math_parallel_coding_solve_vote')
async def math_parallel_coding_solve_vote(system_input: str):
