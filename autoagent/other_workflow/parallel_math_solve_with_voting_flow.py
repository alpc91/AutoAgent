...
async def on_start(event: EventInput, global_ctx):
    print("start the workflow:" + 'parallel_math_solve_with_voting')
@default_drive.listen_group([on_start])
async def solve_analytically(event: EventInput, global_ctx):
    inputs = [{'key': 'math_problem', 'description': 'The math problem statement provided by the user.'}]
    input_dict = dict()
    for inp in inputs: 
    ...
    outputs = [{'key': 'analytical_solution', 'description': 'Mathematical solution generated by analytical methods.', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
...
@default_drive.listen_group([on_start])
async def solve_computationally(event: EventInput, global_ctx):
    inputs = [{'key': 'math_problem', 'description': 'The math problem statement provided by the user.'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    ...
    messages = global_ctx.get('messages', [])
    task = 'Solve the math problem using computational methods'
    outputs = [{'key': 'code_solution', 'description': 'Mathematical solution generated through code implementation.', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
    ...
    for output in outputs:
        ...
        if ans:
            ...
            return result
    raise Exception("No valid answer found")
@default_drive.listen_group([solve_analytically, solve_computationally])
async def aggregate_solutions(event: EventInput, global_ctx):
    inputs = [{'key': 'analytical_solution', 'description': 'Mathematical solution generated by analytical methods.'}, {'key': 'code_solution', 'description': 'Mathematical solution generated through code implementation.'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    ...
    outputs = [{'key': 'final_answer', 'description': 'Final solution determined by majority voting.', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
...
@register_workflow(name = 'parallel_math_solve_with_voting')
async def parallel_math_solve_with_voting(system_input: str):
