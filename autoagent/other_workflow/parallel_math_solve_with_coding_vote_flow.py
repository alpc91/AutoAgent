...
async def on_start(event: EventInput, global_ctx):
    print("start the workflow:" + 'parallel_math_solve_with_coding_vote')
@default_drive.listen_group([on_start])
async def math_solve_event(event: EventInput, global_ctx):
    inputs = [{'key': 'math_problem', 'description': 'The math problem statement'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    
    messages = global_ctx.get('messages', [])
    task = 'Solve the math problem analytically'
    outputs = [{'key': 'math_solution', 'description': 'Analytical solution from Math Solver Agent', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
...
@default_drive.listen_group([on_start])
async def coding_solve_event(event: EventInput, global_ctx):
    inputs = [{'key': 'math_problem', 'description': 'The math problem statement'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    ...
    messages = global_ctx.get('messages', [])
    task = 'Solve the math problem through code implementation'
    outputs = [{'key': 'code_solution', 'description': 'Computational solution from Coding Agent', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
    ...
    for output in outputs:
        ...
        if ans:
            ...
            return result
    raise Exception("No valid answer found")
@default_drive.listen_group([math_solve_event, coding_solve_event])
async def vote_aggregate_event(event: EventInput, global_ctx):
    inputs = [{'key': 'math_solution', 'description': 'Analytical solution from Math Solver Agent'}, {'key': 'code_solution', 'description': 'Computational solution from Coding Agent'}]
    input_dict = dict()
    for inp in inputs: 
        input_dict[inp["key"]] = global_ctx.get(inp["key"], None)
    ...
    outputs = [{'key': 'final_answer', 'description': 'Final answer determined by majority vote', 'condition': None, 'action': {'type': 'RESULT', 'value': None}}]
...
@register_workflow(name = 'parallel_math_solve_with_coding_vote')
async def parallel_math_solve_with_coding_vote(system_input: str):
